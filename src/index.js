...
const sentryLibraries = {
  Sentry: loadable.lib(() => import(/* webpackChunkName: "s_entry-browser" */ '@sentry/browser')),
  SentryIntegrations: loadable.lib(() => import('@sentry/integrations')),
};

const loadSentry = () => {
  const loaders = Object.entries(sentryLibraries).map(([name, Lib]) => new Promise((resolve) => Lib.load().then((mod) => resolve([name, mod]))));
  Promise.all(loaders).then((libs) => {
    const libraries = Object.assign({}, ...libs.map(([name, lib]) => ({ [name]: lib })));
...
const applyConfig = (config) => {
  const errorHandler = (error) => Sentry.load().then((mod) => mod.captureException(error));
  ...
  config.settings.storeExtenders = [...(config.settings.storeExtenders || []), (stack) => [crashReporter, ...stack]];
