...
const SentryBrowser = loadable.lib(() => import(/* webpackChunkName: "s_entry-browser" */ '@sentry/browser'));

// Only load @sentry/node on the server side
const SentryNode = __SERVER__ ? loadable.lib(() => import(/* webpackChunkName: "s_entry-node" */ '@sentry/node')) : null;

const crashReporter = (store) => (next) => (action) => {
  try {
    return next(action);
  } catch (error) {
    if (__SENTRY__?.SENTRY_DSN || process?.env?.RAZZLE_SENTRY_DSN || window?.env?.RAZZLE_SENTRY_DSN) {
